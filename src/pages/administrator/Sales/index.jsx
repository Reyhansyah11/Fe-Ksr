                        ...
                        <div ref={printComponentRef} className="p-6">
                            ...
                            {reportData.sales?.length > 0 ? (
                                <>
                                            ...
                                            <tbody>
                                                {reportData.sales.map((sale) => (
                                                    <tr key={sale.penjualan_id} className="border-b">
                                                        ...
                                                        <td className="px-4 py-2">{sale.user.nama_lengkap}</td>
                                                        <td className="px-4 py-2">
                                                            {sale.pelanggan ? 
                                                                `${sale.pelanggan.nama_pelanggan} ${sale.pelanggan.is_member ? '(Member)' : ''}` : 
                                                                'Umum'}
                                                        </td>
                                                        <td className="px-4 py-2 text-right">{formatRupiah(sale.total)}</td>
                                                        <td className="px-4 py-2 text-right">{(sale.diskon * 100)}%</td>
                                                        <td className="px-4 py-2 text-right">{formatRupiah(sale.total_akhir)}</td>
                                                    </tr>
                                                ))}
                                            </tbody>
                                            <tfoot>
                                                <tr className="font-bold bg-gray-50">
                                                    <td colSpan={5} className="px-4 py-2 text-right">Total Penjualan:</td>
                                                    <td className="px-4 py-2 text-right">
                                                        {formatRupiah(reportData.summary.totalPenjualan)}
                                                    </td>
                                                </tr>
                                                <tr className="font-bold bg-gray-50">
                                                    <td colSpan={5} className="px-4 py-2 text-right">Total HPP:</td>
                                                    <td className="px-4 py-2 text-right">
                                                        {formatRupiah(reportData.summary.totalHPP)}
                                                    </td>
                                                </tr>
                                                <tr className="font-bold bg-gray-50 text-green-600">
                                                    <td colSpan={5} className="px-4 py-2 text-right">Total Laba:</td>
                                                    <td className="px-4 py-2 text-right">
                                    ...
                                    <div className="mt-4">
                                        <p className="text-sm text-gray-600">Total Transaksi: {reportData.sales.length}</p>
                                        <p className="text-sm text-gray-600">Total Qty Terjual: {reportData.summary.totalQty}</p>
                                    </div>
                                </>
                            ) : (
